...
<section class="bg-[#f2ede4] min-h-screen px-4 md:px-10 py-10" x-data>
    ...
    {{-- Filter Header --}}
    <form method="GET" action="{{ route('shop') }}" class="mb-4 px-10 md:px-8">
        @php
            ...
            <div class="flex flex-wrap items-center gap-4">
                ...
                <div x-data="{ open: false }" class="relative">
                    <button type="button" @click="open = !open" class="{{ $filterClass }} flex items-center gap-1">
                        Scents <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div x-show="open" @click.outside="open = false" class="absolute left-0 mt-2 bg-white border rounded shadow w-40 z-10">
                        <ul class="text-sm">
                            <li><a href="{{ route('shop', array_merge(request()->except('aroma'), ['aroma' => ''])) }}" class="block px-3 py-2 hover:bg-gray-100">All</a></li>
                ...
                <div x-data="{ open: false }" class="relative">
                    <button type="button" @click="open = !open" class="{{ $filterClass }} flex items-center gap-1">
                        Gender <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    <div x-show="open" @click.outside="open = false" class="absolute left-0 mt-2 bg-white border rounded shadow w-40 z-10">
                        <ul class="text-sm">
                            <li><a href="{{ route('shop', array_merge(request()->except('gender'), ['gender' => ''])) }}" class="block px-3 py-2 hover:bg-gray-100">All</a></li>
                ...
                <div x-data="{ open: false }" class="relative">
                    <button type="button" @click="open = !open" class="{{ $filterClass }} flex items-center gap-1">
                        Type <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    <div x-show="open" @click.outside="open = false" class="absolute left-0 mt-2 bg-white border rounded shadow w-48 z-10">
                        <ul class="text-sm">
                            <li><a href="{{ route('shop', array_merge(request()->except('type'), ['type' => ''])) }}" class="block px-3 py-2 hover:bg-gray-100">All</a></li>
                            @foreach ([
                            'EDP' => 'Eau De Parfum',
                            'EDT' => 'Eau De Toilette',
                            'BodyMist' => 'Body Mist',
                ...
                <div x-data="{ open: false }" class="relative">
                    <button type="button" @click="open = !open" class="{{ $filterClass }} flex items-center gap-1">
                        Volume <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    <div x-show="open" @click.outside="open = false" class="absolute left-0 mt-2 bg-white border rounded shadow w-48 z-10">
                        <ul class="text-sm">
                            <li><a href="{{ route('shop', array_merge(request()->except('volume'), ['volume' => ''])) }}" class="block px-3 py-2 hover:bg-gray-100">All</a></li>
                            <li><a href="{{ route('shop', array_merge(request()->except('volume'), ['volume' => 'small'])) }}" class="block px-3 py-2 hover:bg-gray-100 {{ request('volume') == 'small' ? 'font-semibold' : '' }}">Small (&lt;30ml)</a></li>
                            <li><a href="{{ route('shop', array_merge(request()->except('volume'), ['volume' => 'medium'])) }}" class="block px-3 py-2 hover:bg-gray-100 {{ request('volume') == 'medium' ? 'font-semibold' : '' }}">Medium (30–60ml)</a></li>
                            <li><a href="{{ route('shop', array_merge(request()->except('volume'), ['volume' => 'large'])) }}" class="block px-3 py-2 hover:bg-gray-100 {{ request('volume') == 'large' ? 'font-semibold' : '' }}">Large (&gt;60ml)</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        ...
                        <div x-show="open" @click.outside="open = false" class="absolute right-0 mt-2 bg-white border rounded shadow w-48 z-10">
                            <ul class="text-sm">
                                <li><a href="{{ route('shop', array_merge(request()->except('sort'), ['sort' => ''])) }}" class="block px-3 py-2 hover:bg-gray-100">Newest</a></li>
                                <li><a href="{{ route('shop', array_merge(request()->except('sort'), ['sort' => 'price_asc'])) }}" class="block px-3 py-2 hover:bg-gray-100">Lowest Price</a></li>
                                <li><a href="{{ route('shop', array_merge(request()->except('sort'), ['sort' => 'price_desc'])) }}" class="block px-3 py-2 hover:bg-gray-100">Highest Price</a></li>
                                <li><a href="{{ route('shop', array_merge(request()->except('sort'), ['sort' => 'name_asc'])) }}" class="block px-3 py-2 hover:bg-gray-100">Name A-Z</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
